...
import { ThemeColor } from "@/lib/features/theme/themeSlice";
...
const ThemeColorPicker: React.FC = () => {
  ...
  const colors: { name: ThemeColor; icon: React.ReactNode }[] = [
    { name: "zinc", icon: <Icon icon={FaFire} className='text-red-500' /> },
    { name: "slate", icon: <Icon icon={FaLeaf} className='text-green-500' /> },
    { name: "stone", icon: <Icon icon={FaTint} className='text-blue-500' /> },
    { name: "neutral", icon: <Icon icon={FaPaintBrush} className='text-gray-500' /> },
  ];
